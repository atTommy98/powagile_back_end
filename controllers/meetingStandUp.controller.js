...
const MeetingStandUp = db.meetingStandUp;
...
exports.create = (req, res) => {
  ...
  // Create a new entry
  const meetingStandUp = new MeetingStandUp(req.body);
...
exports.update = (req, res) => {
  ...
  MeetingStandUp.findByIdAndUpdate(id, req.body, { useFindAndModify: false })
    .then((data) => {
      if (!data) {
        res.status(404).send({
...
exports.delete = (req, res) => {
  ...
  MeetingStandUp.findByIdAndRemove(id, { useFindAndModify: false })
    .then((data) => {
      if (!data) {
        res.status(404).send({
...
exports.getMeetingByDate = async (req, res) => {
  try {
    ...
    const meetings = await MeetingStandUp.find({
      meetingStartTime: {
        $gte: from,
        $lt: to,
      },
